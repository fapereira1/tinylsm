...
static int ikey_cmp(slice_t ka, uint64_t sa, slice_t kb, uint64_t sb) {
  ...
  if (r != 0)
    return r;
  if (sa > sb)
    return -1; /* seq maior → mais novo → vem primeiro */
  if (sa < sb)
    return 1;
  return 0;
}
...
static sl_node_t *sl_find_prev(skiplist_t *sl, slice_t key, uint64_t seq,
                               sl_node_t **prev) {
  ...
  for (int level = sl->max_height - 1; level >= 0; level--) {
    while (1) {
      ...
      if (!next)
        break;
      if (ikey_cmp(next->key, next->seq, key, seq) >= 0)
        break;
      cur = next;
    }
    if (prev)
...
static lsm_status_t sl_insert(skiplist_t *sl, slice_t key, slice_t value,
                              sl_op_t op) {
  if (slice_is_empty(key))
    return LSM_INVALID_ARG;
...
int sl_iter_valid(const sl_iter_t *it) { return it->cur != NULL; }
...
lsm_status_t sl_get_at_seq(skiplist_t *sl, slice_t key, uint64_t max_seq,
  ...
  /*
   * Seek para (key, max_seq) — pela nossa ordenação (key ASC, seq DESC),
   * o primeiro nó >= (key, max_seq) tem seq ≤ max_seq para a mesma key.
   * Isso exclui automaticamente versões escritas após o snapshot.
   */
  sl_node_t *prev = sl_find_prev(sl, key, max_seq, NULL);
  sl_node_t *n = sl_node_fwd(prev)[0];
